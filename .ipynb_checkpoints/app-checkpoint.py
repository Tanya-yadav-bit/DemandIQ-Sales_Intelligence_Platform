...
# ═══════════════════════════════════════════════════════════════════════════
#  MODELING & PREDICTION PIPELINE (UNMODIFIED LOGIC)
# ═══════════════════════════════════════════════════════════════════════════
...
@st.cache_data
def train_model(series_df):
    df = create_features(series_df)
    feature_cols = ["Lag_1", "Lag_2", "Lag_3", "Rolling_Mean_3", "Month_Num", "Quarter_Num"]
    ...
    return model, train, test, preds, mae, rmse, feature_cols
...
def run_clustering(df):
    ...
    growth["Growth_Rate"] = ((growth["Second_Half"] - growth["First_Half"]) / growth["First_Half"].replace(0, 1)) * 100
    ...
    cluster_df = sub_sales.merge(growth[["Growth_Rate"]], left_on="Sub-Category", right_index=True, how="left")
    ...
    label_map = {0: "Stars ⭐", 1: "Growth 🚀", 2: "Stable 🛡️", 3: "Niche 🎯"}
...
def page_forecast_explorer(df):
    ...
    with st.spinner("Executing modeling features & training predictive XGBoost regressor..."):
        model, train_df, test_df, preds, mae, rmse, feat_cols = train_model(monthly)
...
def page_product_segments(df):
    ...
    with st.spinner("Running clustering calculations & executing Principal Component Analysis..."):
        cluster_df, explained = run_clustering(df)

    c1, c2, c3 = st.columns(3)
    with c1:
        kpi_card("Cohorts Analyzed", f"{len(cluster_df)}", "product sub-categories", "Total count of unique product sub-categories")
    with c2:
        kpi_card("Target Clusters", "4 Groups", "K-Means (k=4)", "Number of segment divisions configured in standard clustering")
    with c3:
